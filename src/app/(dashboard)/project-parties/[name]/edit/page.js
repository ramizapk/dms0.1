...
export default function EditProjectPartyPage() {
    const { t, isRTL } = useI18n();
    const router = useRouter();
    const params = useParams();
    const { showToast } = useToast();

    const partyName = decodeURIComponent(params.name);
    ...
    useEffect(() => {
        async function fetchParty() {
            try {
                setLoading(true);
                const response = await api.getProjectPartyProfile(partyName);
                const party = response?.message?.data || response?.data;
                if (party) {
                    setFormData({
                        party_name_arabic: party.party_name_arabic || '',
                        party_name_english: party.party_name_english || '',
                        party_type: party.party_type || '',
                        company_logo: party.company_logo || '',
                    });
                } else {
                ...
                console.error('Failed to fetch project party', err);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.error('Failed to update project party', err);
            showToast(err.message || t('common.error'), 'error');
        } finally {
            setSubmitting(false);
        }
                ...
                fallback={
                    <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-8">
                        ...
                        <p className="text-slate-500 max-w-md mx-auto mb-8">
                            You do not have permission to edit a project party. Please contact your administrator.
                        </p>
                        <Link
            ...
            >
                <div className="space-y-12 pb-20">
                    ...
                    <div className="relative mb-12">
                        ...
                        <PageHeader
                            title={t('project_parties.edit_party')}
                            subtitle={t('project_parties.edit_form_description')}
                        />
                    </div>
                    ...
                    >
                        <form onSubmit={handleSubmit} className="premium-card p-8 sm:p-12 space-y-8">
                            <div className="flex items-center gap-4 pb-6 border-b border-slate-100 mb-8">
                                ...
                                <div>
                                    <h3 className="text-xl font-black text-slate-900">{t('project_parties.edit_party')}</h3>
                                    <p className="text-sm text-slate-500 font-medium">{partyName}</p>
                                </div>
                            </div>
